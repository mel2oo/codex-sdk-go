...
// CurrentTimeRead returns the current Unix timestamp in seconds.
func (h AutoApproveHandler) CurrentTimeRead(ctx context.Context, params protocol.CurrentTimeReadParams) (*protocol.CurrentTimeReadResponse, error) {
	resp := protocol.CurrentTimeReadResponse{CurrentTimeAt: int(time.Now().Unix())}
	return &resp, nil
}
