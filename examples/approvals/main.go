...
func main() {
	...
	prompt := "Review the last commit and summarize"

	client, err := codex.NewClient(ctx, exampleOptions(prompt, logger)...)
	if err != nil {
		panic(err)
	}
	defer client.Close()
...
func exampleOptions(prompt string, logger *slog.Logger) []codex.Option {
	...
	return []codex.Option{
		codex.WithTransport(rpc.NewReplayTransport(exampleTranscript(info, prompt, "Approved summary"))),
