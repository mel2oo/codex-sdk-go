...
func exportSchemas(codexRoot, outDir string) error {
	cmd := exec.Command("cargo", "run", "-p", "codex-app-server-protocol", "--bin", "export", "--", "--out", outDir, "--experimental")
...
func repoRoot() (string, error) {
	...
	return "", fmt.Errorf("unable to locate codex-go-sdk repo root from %s", cwd)
}
...
func codexRepoRoot(sdkRoot string) (string, error) {
	...
	for current := sdkRoot; ; current = filepath.Dir(current) {
		if exists(filepath.Join(current, "codex-rs")) {
			return current, nil
		}
		parent := filepath.Dir(current)
		if parent == current {
	...
	return "", fmt.Errorf("could not find codex checkout above %s (set %s)", sdkRoot, codexRepoRootEnv)
}
