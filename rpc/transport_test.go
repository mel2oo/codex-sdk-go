...
func TestSpawnStdioWithOptionsPassesEnvAndCwd(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell env/cwd test is unix-only")
	}

	cwd := t.TempDir()
	transport, err := SpawnStdioWithOptions(context.Background(), "/bin/sh", []string{"-c", "printf '%s|%s\\n' \"$PWD\" \"$CODEX_GO_SDK_TEST\""}, SpawnStdioOptions{
		Env: []string{"CODEX_GO_SDK_TEST=ok"},
		Cwd: cwd,
	})
	if err != nil {
		t.Fatalf("SpawnStdioWithOptions error: %v", err)
	}
