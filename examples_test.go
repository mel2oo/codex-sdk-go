...
func ExampleQuickstart() {
	...
	client, err := codex.NewClient(
		ctx,
		codex.WithTransport(rpc.NewReplayTransport(exampleTranscript(clientInfo, prompt, "Hello from replay"))),
		codex.WithClientInfo(clientInfo),
	)
	if err != nil {
		panic(err)
	}
...
func ExampleStreaming() {
	...
	client, err := codex.NewClient(
		ctx,
		codex.WithTransport(rpc.NewReplayTransport(exampleTranscript(clientInfo, prompt, "Streaming hello"))),
		codex.WithClientInfo(clientInfo),
	)
	if err != nil {
		panic(err)
...
func exampleTranscript(clientInfo protocol.ClientInfo, prompt, finalResponse string) []rpc.TranscriptEntry {
	return []rpc.TranscriptEntry{
		writeLine(rpc.JSONRPCRequest{
			...
			Method: "initialize",
			Params: mustRaw(initializeParams(clientInfo)),
		}),
...
func initializeParams(clientInfo protocol.ClientInfo) protocol.InitializeParams {
