...
func TestRunStreamedInvalidJSONOptions(t *testing.T) {
	ctx := context.Background()
	client, err := NewClient(ctx, WithTransport(rpc.NewReplayTransport(initializeTranscript())))
	if err != nil {
...
func TestNewClientUsesDefaultClientInfo(t *testing.T) {
	ctx := context.Background()
	client, err := NewClient(ctx, WithTransport(rpc.NewReplayTransport(initializeTranscript())))
	if err != nil {
		t.Fatalf("new client error: %v", err)
...
func TestNewClientSpawnError(t *testing.T) {
	ctx := context.Background()
	_, err := NewClient(ctx, WithStdio(), WithCodexPath("codex-missing-binary"))
	if err == nil {
		t.Fatalf("expected spawn error")
	}
}

func TestNewClientSpawnSurvivesInitContextCancellation(t *testing.T) {
	...
	client, err := NewClient(
		ctx,
		WithStdio(),
		WithCodexPath(writeFakeCodexBinary(t)),
...
func TestStartThreadOnUninitializedClient(t *testing.T) {
	_, err := (&Client{}).StartThread(context.Background(), ThreadStartOptions{})
...
func TestResumeThreadOnUninitializedClient(t *testing.T) {
	_, err := (&Client{}).ResumeThread(context.Background(), ThreadResumeOptions{ThreadID: "thr_123"})
	if err == nil || err.Error() != "codex client is not initialized" {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestNewClientLogsCodexVersionMismatch(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("spawn script test is unix-only")
	}
...
func (h *testServerRequestHandler) CurrentTimeRead(ctx context.Context, params protocol.CurrentTimeReadParams) (*protocol.CurrentTimeReadResponse, error) {
	return nil, nil
}
